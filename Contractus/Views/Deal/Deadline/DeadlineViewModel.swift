...
final class DeadlineViewModel: ViewModel {
    ...
    private func update(deadline: Date) async throws -> Deal {
        try await withCheckedThrowingContinuation { continuation in
            dealService?.update(dealId: state.deal.id, data: UpdateDeal(amount: nil, checkerAmount: nil, prepaymentAmount: nil, ownerBondAmount: nil, contractorBondAmount: nil, deadline: deadline, allowHolderMode: nil), completion: { result in
