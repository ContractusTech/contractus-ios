//  https://github.com/JonasGessner/JGProgressHUD-SwiftUI
...
public final class JGProgressHUDCoordinator: ObservableObject {
    fileprivate var withCancelButton: Bool = false
    fileprivate var cancelPressedAction: (() -> Void)?
    fileprivate let wantsPresentationPublisher = PassthroughSubject<Bool, Never>()

    fileprivate var constructor: (() -> CHUD)? {
        willSet {
     ...
     */
    public func showHUD(constructor: @escaping () -> CHUD, withCancelButton: Bool = false, cancelPressed: (() -> Void)? = nil, handleExistingHUD: (CHUD) -> Bool = { _ in return false }) {
        self.withCancelButton = withCancelButton
        self.cancelPressedAction = cancelPressed
    ...
    public fileprivate(set) var presentedHUD: CHUD?
...
public final class CHUD: JGProgressHUD {

    private var cancelGesture: UITapGestureRecognizer?
    private var cancelPressedActionAdded: Bool = false

    var cancelPressedAction: (() -> Void)? {
        didSet {
            if cancelPressedAction != nil {
                guard !cancelPressedActionAdded else { return }
                cancelPressedActionAdded = true
                self.detailTextLabel.text = R.string.localizable.commonCancel()
                self.detailTextLabel.isUserInteractionEnabled = true
                self.detailTextLabel.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(cancelPress)))
            } else {
                guard cancelPressedActionAdded else { return }
                cancelPressedActionAdded = false
                self.detailTextLabel.text = nil
                self.detailTextLabel.isUserInteractionEnabled = false
                if let cancelGesture = cancelGesture {
                    self.detailTextLabel.removeGestureRecognizer(cancelGesture)
                }
